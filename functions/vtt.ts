// functions/vtt.ts  (repo root)
export const onRequest: PagesFunction = async () => {
  const target = 'https://play.questwithwasem.com/';

  // 3s timeout so users aren’t waiting
  ...
  try {
    ...
    if (res.ok) {
      // Foundry reachable -> send them there
      return Response.redirect(target, 302);
    }
    // Non-OK -> show offline page
...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <p>The virtual table top isn’t reachable right now. It’s usually because the tunnel/host isn’t open.</p>
      <div class="row">
